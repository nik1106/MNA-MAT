            ...
            case{'R'}
                ...
                elseif dist_R==2        %Uniform distribution
                    if(w_R>0)
                        ...
                        %Create a uniform distribution object with lower value = a, and upper value = b
    ...
    switch(solver_flag)
        case{0}
            if(i_i==1) %If first run
                ...
                eval(variables);
                %----------------------------------------------
                %Create a row vector var of the variables - to be used in solve at each run
                ...
                eval(equations);
                %----------------------------------------------
                %Create a row vector eqn_solve for the equation symbolic variables
            ...
            if(i_i==1) %If first run, create empty text file Results.txt and write date and time along with file name, resistor distribution information and number of runs
            ...
            if(i_i==runs) %If last run
                type('Results.txt');  %Display contents of Results.txt text file
                fclose(F); %Close Results.txt text file
                [m,n] = size(table);
                x = 1:m;
                ...
                for l=1:num_CCVS  %Plot each CCVS current recorded at each run
                    figure;
                    quant = table(:,(num_Nodes+num_V+num_VCVS+l));
                    stem(x,quant);
                    xlabel('RUN');
                    ylabel(['i\_' CCVS(l).Name]);
                end
            end
            %%--------------------------------------------------------------------------
        case{1}
            ...
            eval(var_string);
            %-----------------------------------------------
            %Convert the equations (only LHS) to a form suitable for daeFunction
            %Use the converted equations to make a row vector eqn_daeFunction - to be used in daeFunction
            ...
            if((num_V~=0)||(num_VCVS~=0)||(num_CCVS~=0)||(num_L~=0))
                figure(2);
                plot(t,v(:,(num_Nodes+1):end)); hold on; %Plot the currents through voltage sources and inductors as a function of time
                if(i_i==1)
            ...
            if(i_i==runs) %If last run
                type('MC_values_ode.txt');  %Display the passive element values used at each run (display contents of MC_values_ode.txt text file)
