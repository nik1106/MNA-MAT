...
dist = input('Choose Distribution:\n 1. Normal \n 2. Uniform \n');
runs = input('Number of trials to be performed: \n');
if dist == 1
    display('Mean is the original value of the component in the netlist');
    SD = input('Set Standard Deviation in % : \n');
else
    w = input('Set window on either side (in %) : \n');
end
for i_i=1:runs
    display(i_i);
    ...
    for i=1:length(Name)
        switch(Name{i}(1))
            case{'R','L','C'}
                ...
                if dist == 1
                    SD_element = SD * Element(num_Elements).Value / 100;
                    ...
                    a = Element(num_Elements).Value - (Element(num_Elements).Value)*w/100;
                    b = Element(num_Elements).Value + (Element(num_Elements).Value)*w/100;
    ...
    switch(solver_flag)
        case{0}
            ...
            if(i_i==1) %If first run, create empty text file Results.txt and write date and time along with file name and distribution
                ...
                fprintf(F,'\n');
                if(dist==1)
                    fprintf(F,'Distribution : Normal');
                    fprintf(F,'\n');
                    fprintf(F,['Standard deviation in %% : ' num2str(SD)]);
                    fprintf(F,'\n');
                else
                    fprintf(F,'Distribution : Uniform');
                    fprintf(F,'\n');
                    fprintf(F,['Window size in %% : ' num2str(w)]);
                    fprintf(F,'\n');
                end
        ...
        case{1}
            ...
            if(i_i==1)
                ...
                if(dist==1)
                    xlswrite('Results.xls',{'Distribution : Normal'},1,'A3');
                    xlswrite('Results.xls',{['SD in % : ' num2str(SD)]},1,'A4');
                else
                    xlswrite('Results.xls',{'Distribution : Uniform'},1,'A3');
                    xlswrite('Results.xls',{['Window size % : ' num2str(w)]},1,'A4');
                end
                xlswrite('Results.xls',{['Runs : ' num2str(runs)]},1,'A5');
            end
            writetable(T,'Results.xls','Range','A7','Sheet',i_i);
            figure(1);
