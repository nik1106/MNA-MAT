...
for i_i=1:runs
    ...
    if(solver_flag==0)
        ...
        %%to each nodal KCL equation,VCVS equation and CCVS equation, replace the terms
    ...
    elseif(solver_flag==1)
        ...
        for i=1:num_V %Modify each independent voltage source equation to only LHS [no RHS ('=0')]
    ...
    switch(solver_flag)
        case{0}
            ...
            %%--------------------------------------------------------------------------
        case{1}
            %Create the variables to be used in daeFunction
            variables='syms';
